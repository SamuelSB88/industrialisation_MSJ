...
class Preprocessor:
    ...
    def clean_dataframe(self, df):
        ...
        # Encodage forcé de toutes les colonnes object en codes numériques
        for col in df_clean.columns:
            if df_clean[col].dtype == 'object':
                df_clean[col] = df_clean[col].astype('category').cat.codes

        # Debug : vérifier s'il reste des colonnes object
        remaining_object_cols = df_clean.select_dtypes(include='object').columns.tolist()
        if remaining_object_cols:
            print("Colonnes encore en object après encodage :", remaining_object_cols)
            raise ValueError("Certaines colonnes ne sont pas encodées correctement.")
